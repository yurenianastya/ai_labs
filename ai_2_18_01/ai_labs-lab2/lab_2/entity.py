...
class Agent:
    ...
    def __init__(self, spawn_node, graph, goals):
        ...
        # shooting
        self.shooting_cooldown = 1.0  # Час між пострілами в секундах
    ...
    def update(self, dt, all_agents):
        ...
        self.check_and_shoot(all_agents)
    ...
    def check_and_shoot(self, all_agents):
        detected_agents, _ = self.check_fov(all_agents)
        if detected_agents and self.time_since_last_shot >= self.shooting_cooldown:
            target_agent = detected_agents[0]  # Вибір першої цілі (можна покращити логіку вибору)
            self.shoot(target_agent, all_agents)

    def shoot(self, target_agent, all_agents):
        print(f"Bot at {self.position} shoots at bot at {target_agent.position}!")
        self.time_since_last_shot = 0.0
        utils.SHOTS.append((self.position, target_agent.position, 0.2))
    
    # Зменшуємо HP цільового бота
        if target_agent.armor > 0:  # Якщо є броня, вона поглинає частину урону
            damage_to_armor = min(20, target_agent.armor)
            target_agent.armor -= damage_to_armor
            damage_to_health = 20 - damage_to_armor
        else:  # Якщо броні немає, весь урон іде на здоров’я
            damage_to_health = 20
        target_agent.health -= damage_to_health
    
    # Перевірка на видалення бота
        if target_agent.health <= 0:
            target_agent.health = 0
            print(f"Bot at {target_agent.position} has been eliminated!")
            # Видалення бота зі списку всіх агентів
