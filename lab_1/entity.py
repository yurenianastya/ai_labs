...
class Player():
    ...
    def get_movement_keys(self, keys, current_time):
        ...
        if self.check_collision_with_obstacles():
            self.position = original_position  # Якщо є зіткнення, повертаємо назад
    ...
    def update_bullets(self):
        for bullet in self.bullets[:]:
            ...
            self.check_bullet_collision(zombies)

    def check_bullet_collision(self, zombies):
        for zombie in zombies:
            if zombie.is_dead:  # Якщо зомбі мертвий, пропускаємо перевірку
                continue
            for bullet in self.bullets:
                distance = pygame.Vector2(bullet["pos"][0], bullet["pos"][1]).distance_to(zombie.position)
                if distance < zombie.radius:
                   zombie.kill(pygame.time.get_ticks())  # Вбиваємо зомбі, якщо потрапили
...
class Zombie:
    ...
    def respawn(self, current_time):
        ...
        if self.is_dead and current_time - self.death_time >= 5000:  
            self.is_dead = False
