...
class PickupItem:
    def __init__(self, x, y, item_type, item_id):
        self.x = x
        self.y = y
        self.item_type = item_type
        self.id = item_id  # Додаємо id
        self.respawn_time = None  # Не використовуємо для цієї частини
    ...
    def respawn(self, graph):
        new_node = random.choice(list(graph.nodes.values()))
        self.x, self.y = new_node.x, new_node.y        

def check_bot_on_item(bot, items, graph):
    for item in items:
        if abs(bot.position[0] - item.x) <= 3 and abs(bot.position[1] - item.y) <= 3:
            if item.item_type == "health":
                bot.health = min(bot.max_health, bot.health + 20)  # Збільшуємо здоров'я, але не більше max_health
                print(f"Бот {bot} отримав +20 здоров'я. Поточне: {bot.health}")
            elif item.item_type == "armor":
                bot.pick_up_armor()  # Збільшуємо броню
                print(f"Бот {bot} отримав броню.")
...
for i in range(20):
    ...
    PICKUP_ITEMS.append(PickupItem(health_node.x, health_node.y, "health", f"health_{i}"))
    ...
    PICKUP_ITEMS.append(PickupItem(armor_node.x, armor_node.y, "armor", f"armor_{i}"))
...
def draw_hit_bars(bot, surface):
    ...
    pygame.draw.rect(surface, (100, 100, 100), (health_bar_x, health_bar_y, bar_width, bar_height))  # Сірий фон
    ...
    pygame.draw.rect(surface, (50, 50, 50), (health_bar_x, armor_bar_y, bar_width, bar_height))  # Сірий фон
    pygame.draw.rect(surface, (0, 200, 255), (health_bar_x, armor_bar_y, int(bar_width * armor_percent), bar_height))  # Синій індикатор
...
while running:
    ...
    for bot in BOTS:
        bot.update(dt)
        check_bot_on_item(bot, PICKUP_ITEMS, map_graph)
    ...
    # Перевірка, чи бот потрапив на предмет
